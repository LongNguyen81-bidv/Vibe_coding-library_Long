...
const EditBookPage = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.data.success) {
        toast.success(response.data.message || 'Cập nhật sách thành công!')
        navigate(`/books/${id}`)
      }
    } catch (error) {
      const errorMessage = error.response?.data?.message || 'Cập nhật sách thất bại'
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Sửa Thông Tin Sách</h1>
            <p className="text-gray-600 mt-1">Chỉnh sửa thông tin sách trong kho thư viện</p>
          </div>
        </div>
      </div>
